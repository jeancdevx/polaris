...
The same config using the `global` key. The `.env` files move to
`global.inputs`, which means they get folded into each task's hash individually
rather than the global hash. This lets tasks exclude specific `.env` files if
needed.
...
With this approach, a task that doesn't care about `.env.production` can exclude
it:
...
This wouldn't have been possible with `globalDependencies`, where
`.env.production` would be baked into the global hash and affect every task
unconditionally.
