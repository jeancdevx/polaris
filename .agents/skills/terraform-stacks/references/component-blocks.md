...
Complete reference for all blocks available in Terraform Stack component
configuration files (`.tfcomponent.hcl`).
...
- **type** (required): Data type (string, number, bool, list, map, object, set,
  tuple, any)
...
- **alias** (label 2, required): Unique identifier for this provider
  configuration
...
For complete provider examples including for_each and multi-cloud patterns, see
`examples.md`.
...
- **source** (required): Module source (see [Source Argument](#source-argument)
  below)
...
The `source` argument accepts the same module sources as traditional Terraform
configurations.
...
- **Terraform Enterprise**: Use your instance hostname (e.g.,
  `terraform.mycompany.com`)
- **Generic hostname**: Use `localterraform.com` for deployments spanning
  multiple Terraform Enterprise instances
...
The `version` argument is supported only for registry-based sources (public and
private registries). Local file paths and Git sources do not support the
`version` argument.
...
**Note**: Modules sourced from local file paths always share the same version as
their caller and cannot have independent version constraints.
...
For complete component examples including for_each, multi-region, public
registry, and multi-provider patterns, see `examples.md`.
...
For complete output examples including sensitive outputs and for expressions,
see `examples.md`.
