...
**Issue:** Component A references Component B, and Component B references
Component A.
...
**Solution:** Remove `validation` blocks from variable declarations. Stacks do
not support validation blocks:
...
**Solution:** Always specify `type` for variables - it's required in Stacks
(unlike traditional Terraform):
...
1. **Remove provider blocks from modules** - configure providers in Stack
   configuration instead
2. **Use modules that don't contain provider blocks** (most public registry
...
**Issue:** The HCP Terraform UI doesn't provide an option to destroy Stack
deployments.

**Why:** Stack deployment destruction is only available through configuration,
not the UI.

**Solution:** Set `destroy = true` in the deployment block and upload the
configuration:
...
**Important:** You cannot destroy deployments from the UI. This is by design to
...
3. **Provider version conflict** - Check `.terraform.lock.hcl` matches required
   providers
...
**Issue:** Deployment is waiting for approval but CLI doesn't show approval
prompt.

**Why:** CLI monitoring commands are non-blocking and don't automatically prompt
for approval.
...
- Subject matches:
  `organization:<org>:project:<project>:stack:<stack>:deployment:<deployment>`
...
3. Verify subject pattern matches your organization/project/stack/deployment
   names
...
**Issue:** Modules from the Terraform public registry cause errors during plan
or apply.
...
- `terraform-aws-modules/ecs-service/aws` - May have issues with certain
  configurations
...
2. **Check module compatibility** by reviewing recent issues on the module
   repository
...
**Issue:** Deployment with deferred changes doesn't complete after multiple
iterations.
...
4. **Consider multi-stage deployments** if resources truly can't be created
   together
...
**Issue:** Commands like `terraform stacks deployment-run watch` never return in
CI/CD pipelines.

**Why:** Watch commands stream output indefinitely and are designed for
interactive use.

**Solution:** Use API polling instead of watch commands. See `api-monitoring.md`
...
2. **Wrong artifact name** - use one of: plan-description, plan-debug-log,
   apply-description, apply-debug-log
...
**Issue:** Artifacts endpoint returns redirect response instead of artifact
content.
