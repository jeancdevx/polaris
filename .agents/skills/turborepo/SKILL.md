...
**DO NOT** put task logic in root `package.json` when it can live in packages.
...
packages, such as Vitest Projects' `//#test`, repo-wide release scripts, or
...
`globalDependencies` affects ALL tasks in ALL packages via the **global hash** —
tasks cannot opt out of specific files, even with negation globs in `inputs`. Be
specific.
...
With `futureFlags.globalConfiguration`, this problem is reduced because
`global.inputs` files are folded into each task's inputs (not the global hash).
Tasks can exclude specific files:
...
With `futureFlags.globalConfiguration`, the same config moves global settings
under `global` — and `.env` becomes a per-task input instead of a global hash
input:
