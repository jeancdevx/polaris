...
resource "aws_ecs_service" "admin_service" {
  ...
  load_balancer {
    ...
    container_port   = var.admin_service_container_port
  }
  ...
  depends_on = [
    ...
    aws_secretsmanager_secret_version.admin_service_env
  ]
...
resource "aws_ecs_service" "event_processor_service" {
  name            = local.event_processor_service_name
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.event_processor_service.arn
  desired_count   = var.event_processor_service_desired_count
  launch_type     = "FARGATE"
  ...
  depends_on = [
    aws_secretsmanager_secret_version.event_processor_service_env
  ]
