locals {
  ...
  function_name = coalesce(
    var.function_name,
    "${local.name_prefix}-rfid-validator"
  )
  ...
  lambda_dist_dir = "${local.repository_root}/lambdas/rfid-validator/dist"

  lambda_zip_path = "${local.lambda_module_dir}/.terraform/${local.function_name}.zip"

  lambda_src_hash = sha256(join("", [
    for file_path in sort(fileset("${local.repository_root}/lambdas/rfid-validator", "**")) :
    filesha256("${local.repository_root}/lambdas/rfid-validator/${file_path}")
