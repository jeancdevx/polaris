locals {
  ...
  function_name = coalesce(
    var.function_name,
    "${local.name_prefix}-notification-sender"
  )
  ...
  lambda_dist_dir = "${local.repository_root}/lambdas/notification-sender/dist"

  lambda_zip_path = "${local.lambda_module_dir}/.terraform/${local.function_name}.zip"

  lambda_src_hash = sha256(join("", [
    for file_path in sort(fileset("${local.repository_root}/lambdas/notification-sender", "**")) :
    filesha256("${local.repository_root}/lambdas/notification-sender/${file_path}")
