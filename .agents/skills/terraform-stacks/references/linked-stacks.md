...
Complete reference for linking Terraform Stacks together using published outputs
and upstream inputs.
...
- Must apply the Stack's deployment configuration before downstream Stacks can
  reference outputs
...
For a complete example showing full Stack configurations with all files
(variables, providers, components, outputs, deployments) for both upstream and
downstream Stacks, see the "Linked Stacks (Cross-Stack Dependencies)" section in
`examples.md`.
