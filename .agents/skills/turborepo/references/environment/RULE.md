...
When the `globalConfiguration` future flag is enabled, global environment keys
move under the `global` key with cleaner names:
...
`global.env` and `global.passThroughEnv` behave identically to their top-level
counterparts — they affect the global hash and all tasks, respectively. The
rename is purely organizational.
